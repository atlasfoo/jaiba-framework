...
kind: tooling
role: —
resource: "CLI `cz` (installed in CI via `uv tool install commitizen==4.18.1`) / config at `.cz.toml`"
...
Read `.cz.toml` directly for the current pinned baseline, or (with
`uv` installed — no local Python required, `uv` provisions its own)
run `uvx --from commitizen==4.18.1 cz bump --dry-run` to preview the
next computed version from the commits on the current branch.
...
in CI, where `uv tool install commitizen==4.18.1` provisions it fresh
each run (via [astral-sh/setup-uv](https://github.com/astral-sh/setup-uv),
replacing the earlier `actions/setup-python` + `pip install` pair — see
[references/github-actions.md](github-actions.md)). `--no-raise 21`
must precede `bump` on the CLI invocation:
