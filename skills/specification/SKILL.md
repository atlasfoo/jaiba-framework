---
name: specification
description: Spec-driven workflow for formalizing requirements into PRD + user stories before implementation. Use for multi-faceted requirements that need clarifying. Three modes: brainstorm (shape fuzzy ideas), define (draft PRD/stories), archive (close delivered spec). Never writes code — that's planning's role.
version: 1.0.0
...
to `.ai/specs/`. A spec with no constitution to bound its scope, and
no `planning` skill that could implement it consistently, isn't a spec
this framework can stand behind. Tell the developer plainly what's
missing and route them:
